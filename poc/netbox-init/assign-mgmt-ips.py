#!/usr/bin/env python3
"""
Assign Management IP Addresses to Servers
==========================================
Assigns random IP addresses from each datacenter's management subnet to server mgmt0 interfaces.

IP Allocation by Site:
- DC-East:   10.23.0.1 - 10.23.1.254 (10.23.0.0/23)
- DC-West:   10.23.2.1 - 10.23.3.254 (10.23.2.0/23)
- DC-Center: 10.23.4.1 - 10.23.5.254 (10.23.4.0/23)

Usage:
    docker cp assign-mgmt-ips.py netbox:/tmp/
    docker exec netbox python /tmp/assign-mgmt-ips.py
"""
...
def assign_ips_for_site(site, prefix_str):
    """Assign management IPs to all servers in a site."""
    print(f"\n{site.name}:")
    print(f"  Management Subnet: {prefix_str}")
    ...
    for idx, server in enumerate(servers):
        ...
        try:
            mgmt_interface = Interface.objects.get(device=server, name='mgmt0')
        except Interface.DoesNotExist:
            print(f"  ✗ {server.name}: No mgmt0 interface found")
            skipped += 1
            continue
        ...
        try:
            ...
            ip_address = IPAddress.objects.create(
                address=f"{ip_str}/24",
                status='active',
                dns_name=f"{server.name.lower()}-mgmt",
                description=f"Management for {server.name}"
            )

            # Associate with interface
            ip_address.assigned_object = mgmt_interface
            ip_address.save()
...
def main():
    """Main execution."""
    print("=" * 70)
    print("ASSIGN MANAGEMENT IP ADDRESSES")
    print("=" * 70)
    print("\nAssigning random IPs from each site's management subnet")
    print("=" * 70)

    # Site to subnet mapping
    site_configs = [
        {'slug': 'dc-east', 'prefix': '10.23.0.0/23'},
        {'slug': 'dc-west', 'prefix': '10.23.2.0/23'},
        {'slug': 'dc-center', 'prefix': '10.23.4.0/23'},
    ]
    ...
    print("\n" + "=" * 70)
    print("✓ MANAGEMENT IP ASSIGNMENT COMPLETE!")
    print("=" * 70)
    print(f"\nTotal IPs assigned: {total_assigned}")
    print(f"Total skipped: {total_skipped}")
    print("\nServers now have management IPs from their datacenter's subnet")
    print("=" * 70)
